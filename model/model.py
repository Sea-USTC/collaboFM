...
class cell(nn.Module):
    def __init__(self, module_type="linear", param_list=None):
        ...
        if module_type == "linear":
            ...
            self.proxy = orthogonal(
                feat_dim=param_list[1], class_num=param_list[3])
...
class model_cifar(nn.Module):
    def __init__(self, model_name, encoder_list=["identity"], encoder_para_list=None,
        ...
        if "resnet18" in model_name:
            ...
            self.backbone = SimpleCNN_header(
...
class model_vit(nn.Module):
    def __init__(self, model_name, encoder_list=["identity"], encoder_para_list=None,
        ...
        if model_name == "vittiny_16_224":
            basemodel = timm.create_model('vit_tiny_patch16_224', pretrained=pretrained,
                                          drop_path_rate=0.1, num_classes=head_para_list[3], checkpoint_path=checkpoint_path)
        if model_name == "vitsmall_16_224":
            basemodel = timm.create_model('vit_small_patch16_224', pretrained=pretrained,
                                          drop_path_rate=0.1, num_classes=head_para_list[3], checkpoint_path=checkpoint_path)
        if model_name == "vitbase_16_224":
            basemodel = timm.create_model('vit_base_patch16_224', pretrained=pretrained,
                                          drop_path_rate=0.1, num_classes=head_para_list[3], checkpoint_path=checkpoint_path)
        if model_name == "vitbase_32_224":
            basemodel = timm.create_model('vit_base_patch32_224', pretrained=pretrained,
                                          drop_path_rate=0.1, num_classes=head_para_list[3], checkpoint_path=checkpoint_path)
        if model_name == "vitlarge_16_224":
            basemodel = timm.create_model('vit_large_patch16_224', pretrained=pretrained,
                                          drop_path_rate=0.1, num_classes=head_para_list[3], checkpoint_path=checkpoint_path)
        if model_name == "vitlarge_32_224":
            basemodel = timm.create_model('vit_large_patch32_224', pretrained=pretrained,
                                          drop_path_rate=0.1, num_classes=head_para_list[3], checkpoint_path=checkpoint_path)
...
class model_cliptqn(nn.Module):
    def __init__(self):
        super(model_cliptqn, self).__init__()
        self.tqn = TQN_Model(embed_dim=512, class_num=2)

    def forward(self, image, text,clip_model):
        # image: (b,3,224,224)g
        # text:
    ...
    def forward_tqn(self, image, text_repre, clip_model):
        image_features = clip_model.encode_image(image).float()
        image_features = image_features / image_features.norm(dim=1, keepdim=True)
        image_features = image_features.unsqueeze(1)
        out = self.tqn(image_features,text_repre)
        return out



class model_fedclip(nn.Module):
    def __init__(self, model_name, encoder_list=["linear"], encoder_para_list=None):
        super(model_fedclip, self).__init__()
        d = 512
        self.img_adpt = nn.Sequential(
            nn.Linear(d, d), nn.Tanh(), nn.Linear(d, d), nn.Softmax(dim=-1))
        # self.encoder = nn.Sequential()
        # for idx, module_name in enumerate(encoder_list):
        #     name = "encoder_"+str(idx)
        #     self.encoder.add_module(name, cell(module_name, encoder_para_list))
    ...
    def forward_AttAI(self, image, text, clip_model):
        image_features = clip_model.encode_image(image).float()
        text_features = clip_model.encode_text(text).float()
        # image_features = self.encoder(image_features)
        image_features_att = self.img_adpt(image_features)
